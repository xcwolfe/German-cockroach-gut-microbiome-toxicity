...
```{r}
plot_bar(physeqshannon, fill="Phylum") + geom_bar(aes(color=Phylum, fill=Phylum), stat="identity", position="stack")
```
...
# Set prunescale 
prunescale = 0.0001
seq_depth = sample_sums(physeqshannon)[1]
# Prune out rare OTUs by mean relative abundance set by prunescale
tax.mean <- taxa_sums(physeqshannon)/nsamples(physeqshannon)
...
physeq.prune <- prune_taxa(tax.mean > prunescale*seq_depth, physeqshannon)
